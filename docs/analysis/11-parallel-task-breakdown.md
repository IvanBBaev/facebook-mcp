...
| V02 | Insights: page/post insights, reshape contract (`aggregate`, row caps), doctor metric probe + generated metric table, rename-suggestion table, ≥100-likes + freshness notes; **Reels insights via a separate `facebook_reel_insights` on `/video_insights` (G-TOOL-2, shipped)** | `src/api/insights.ts`, `src/tools/insights.ts`, fixtures | I1 | C6, CC-INS-1..6, G-TOOL-2 | L |
