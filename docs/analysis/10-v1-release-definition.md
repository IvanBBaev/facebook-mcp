...
| Phase 1 exit (reader + insights) | `0.2.0` | none (no publish) |
| Phase 2 exit (posts + media) | `0.3.0` | none (no publish) |
| Phase 3 exit (moderation + messages) | `0.4.0` | none (no publish) |
...
- **G-TOOL-1 `facebook_get_comment` (moderation package).** The catalog has
  `list_comments` but no single-comment read. Moderation previews already
  require re-fetching current comment text (CC-MOD-6); the model needs the
  same capability first-class — acting on a comment ID from a prior turn
  without re-listing the whole thread. RO annotation. Phase 3.
- **G-TOOL-2 Reels insights path.** Reels metrics live on
  `/{video-id}/video_insights`, not on the post-insights edge — the catalog's
  `facebook_post_insights` cannot reach them, which undercuts the "insights
  that work in 2026" differentiator while `create_reel` is flagship surface.
  ✎ Verify the edge at Phase 2, then either add `facebook_reel_insights` or
  route by ID type inside `post_insights` (decide on verified shape). Phase 2.
...
- **G-TOOL-4 `mark seen` on conversations (candidate).** Polling-based
  messaging leaves conversations unread forever, so `unread_count` — the
  designed polling diff signal (doc 03) — never resets. A `sender_action:
  mark_seen` call (folded into `facebook_get_conversation` as an opt-in param,
  not a separate tool) fixes the operator workflow. Decide at Phase 3 entry.
- **G-TOOL-5 Server version surfacing.** `--version` CLI flag and a `version`
  field in `facebook_whoami` output (server version + pinned default
  `FB_API_VERSION` + SDK version). Required for usable bug reports once
  non-author users exist; trivial via the metadata SSOT. Phase 0.

## 3. Runtime additions for 1.0

- **G-RUN-1 Journal rotation.** The write journal (05, Sec #7) grows without
  bound — CC-LIFE-1 covers write *failure* but nothing covers growth. Cap by
  size with simple rotation (`journal.jsonl` → `journal.1.jsonl`, keep one
  generation, ~5 MB default). No config knob unless someone asks. Phase 0.
- **G-RUN-2 Pin `platform=messenger` on conversation reads.** The
  `/conversations` edge can return Instagram threads on linked accounts; IG is
  out of scope (doc 01) and accidental IG data would violate the declared
  scope. Hardcode the platform param; document it. Phase 3.
...
| Item | Phase |
|---|---|
| G-TOOL-5 version surfacing, G-RUN-1 journal rotation | 0 |
| G-TOOL-2 Reels insights ✎, G-TOOL-3 Reels lifecycle ✎ | 2 |
| G-TOOL-1 get_comment, G-TOOL-4 mark_seen decision, G-RUN-2 platform pin | 3 |
| G-DOC-1…6 (hygiene, disclosure, runbooks, policy, compat, scope) | 4 |
| §1 version map | governs all phases |

These items are additive to the roadmap v2 gates: G-TOOL-2/3 join Phase 2's
✎-verification list; the G-DOC set joins the Phase 4 exit gate (a 1.0 without
them fails the "non-author onboarding" spirit of PM #3).
